...
import { LabelModal } from '../cmps/label-modal.jsx'
class _NoteToolbar extends React.Component {
  ...
  hideModal = () => {
    if (this.state.isLabelModal) {
      this.setState({ isLabelModal: false })
    }
  }
  ...
  onToggleLabels = (ev) => {
    ev.stopPropagation();
    this.setState(prevState => ({ isLabelModal: !prevState.isLabelModal }))
  }
  render() {
    const { type } = this.props;
    const { isLabelModal } = this.state;
    return (
      <React.Fragment>
        <div className="note-toolbar flex" style={this.style}>
          ...
          {type === 'existed-note' && <img title="Add label" onClick={ev => this.onToggleLabels(ev)} src="assets/svg/label.svg" />}
        ...
        {isLabelModal && <LabelModal onAddLabel={this.props.onAddLabel} labels={this.props.note.info.labels || []} />}
